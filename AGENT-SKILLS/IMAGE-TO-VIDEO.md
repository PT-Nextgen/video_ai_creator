...
### Hal yang wajib diisi pada wan22_i2v_prompt.json
...
4. `negative_prompt_two`
    Hanya perlu diisi apabila `duration_seconds` 10s
5. Text prompt diisi pada field `id_new` pada field `positive_prompt_one`, `positive_prompt_two`, `negative_prompt_one` dan `negative_prompt_two` 

### Flow pembuatan video untuk `duration_seconds` 5s 
1. Scene ini harus mempunyai minimal satu gambar awal pertama
