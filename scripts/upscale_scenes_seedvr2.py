"""Upscale marked root-scene video outputs through the SeedVR2 workflow."""
...
TEMPLATE_PATH = ROOT / "api_template" / "seedvr2_upscale_api.json"
...
def _load_template() -> dict:
    ...
    if not isinstance(workflow, dict):
        raise ValueError(f"Template SeedVR2 bukan object JSON: {TEMPLATE_PATH}")
    return workflow
...
def _upload_video(server: str, source: Path) -> str:
    ...
    write_log(f"[seedvr2-upscale] Upload video berhasil: {source.name} -> {name}")
    return name
...
def upscale_scene(scene_dir: Path, server: str) -> bool:
    ...
    try:
        ...
        write_log(f"[seedvr2-upscale] {scene_dir.name}: gagal membaca scene_meta.json: {exc}", level="error")
        return False

    if not bool(meta.get("upscale", False)):
        write_log(f"[seedvr2-upscale] {scene_dir.name}: upscale=false, dilewati")
        return True

    source = _latest_root_video(scene_dir)
    if source is None:
        write_log(f"[seedvr2-upscale] {scene_dir.name}: video output root tidak ditemukan", level="error")
        return False

    write_log(f"[seedvr2-upscale] {scene_dir.name}: mulai, sumber={source.name}")
    try:
        ...
        try:
            ...
            if not temporary_path.is_file() or temporary_path.stat().st_size <= 0:
                raise RuntimeError(f"Download hasil SeedVR2 kosong: {temporary_path}")
            ...
            if temporary_path.exists():
                try:
                    ...
                    pass
        write_log(f"[seedvr2-upscale] {scene_dir.name}: selesai, hasil={final_path.name}")
        return True
    except Exception as exc:
        logger.error("[seedvr2-upscale] %s gagal: %s", scene_dir.name, exc)
        write_log(f"[seedvr2-upscale] {scene_dir.name}: gagal: {exc}", level="error")
        return False
...
def run(project: str, server: str, selected_scenes: list[str] | None = None) -> int:
    ...
    if not project_dir.is_dir():
        write_log(f"[seedvr2-upscale] Project tidak ditemukan: {project_dir}", level="error")
        return 1
    ...
    if not scene_dirs:
        write_log(f"[seedvr2-upscale] Tidak ada scene yang cocok pada {project_dir}", level="error")
        return 1

    if project_uses_llama(project_dir):
        controller = RuntimeServiceController.from_config()
        write_log("[seedvr2-upscale] Project memakai Llama; memastikan ComfyUI aktif")
        controller.ensure_comfyui(reason=f"SeedVR2 upscale project={project}")
    else:
        write_log("[seedvr2-upscale] Project memakai Gemini; switch runtime tidak diperlukan")

    for scene_dir in scene_dirs:
        if not upscale_scene(scene_dir, server):
            write_log(f"[seedvr2-upscale] Proses dihentikan setelah gagal pada {scene_dir.name}", level="error")
            return 1
    write_log(f"[seedvr2-upscale] Semua scene selesai diproses: {project_dir.name}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description="SeedVR2 upscale video output root scene yang memiliki upscale=true")
