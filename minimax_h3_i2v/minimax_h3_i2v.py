...
def is_valid_minimax_h3_i2v_prompt(value) -> bool:
    """Return whether text follows the required MiniMax H3 I2VA structure."""
    text = str(value or "").strip()
    if not text.startswith(I2VA_FIRST_FRAME_PREFIX):
        return False
    body = text[len(I2VA_FIRST_FRAME_PREFIX):]
    if not body.startswith("\n\n"):
        return False
    body = body[2:]
    positions = []
    for label in I2VA_REQUIRED_SECTION_LABELS:
        position = text.find(label)
        if position < 0:
            return False
        positions.append(position)
    if positions != sorted(positions) or not body.startswith(I2VA_REQUIRED_SECTION_LABELS[0]):
        return False
    for index, position in enumerate(positions):
        content_start = position + len(I2VA_REQUIRED_SECTION_LABELS[index])
        content_end = positions[index + 1] if index + 1 < len(positions) else len(text)
...
def build_workflow(
    ...
    _set_input(workflow, "134", "expression", frame_expression)
...
def build_minimax_h3_i2v_workflow(
    i2v_prompt: dict,
    scene_meta: dict | None = None,
    uploaded_name: str | None = None,
    duration_override: int | float | None = None,
...
def send_workflow(
    ...
    prompt_message = (
        f"Prompt MiniMax H3 I2V dikirim ke ComfyUI untuk {source_label}:\n"
    ...
    write_log(
        f"Sent minimax_h3_i2v workflow for {source_label}: {json.dumps(result)}",
